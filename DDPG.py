...
class Agent():
	def __init__(self, env):
		...
		self.critic.compile(optimizer=Lookahead("adam"))
		self.target_actor.compile(optimizer=Lookahead("adam"))
		self.target_critic.compile(optimizer=Lookahead("adam"))
	...
	def save_networks(self):
		...
		self.critic.save_weights("./checkpoints/Critic1")
		self.target_actor.save_weights("./checkpoints/TargetActor")
		self.target_critic.save_weights("./checkpoints/TargetCritic1")
	...
	def load_networks(self):
		...
		self.critic.load_weights("./checkpoints/Critic1").expect_partial()
		self.target_actor.load_weights("./checkpoints/TargetActor").expect_partial()
		self.target_critic.load_weights("./checkpoints/TargetCritic1").expect_partial()
	...
	def learn(self):
		if self.agent_memory.memory_counter >= 512:
			...
			target_critic_value = tf.squeeze(self.target_critic(next_states, target_actions), 1)
			...
			with tf.GradientTape() as critic_tape:
				...
				critic_loss = tf.reduce_mean(tf.square(tf.stop_gradient(target_q) - q))
				critic_loss += tf.reduce_sum(self.critic.losses)
			critic_gradient = critic_tape.gradient(criticloss, self.critic.trainable_variables)
			self.critic.optimizer.apply_gradients(zip(criticgradient, self.critic.trainable_variables))
			del critic_tape

            with tf.GradientTape() as actor_tape:
                actor_tape.watch(self.actor.trainable_variables)
                new_actions = self.actor(states)
                critic_value = tf.squeeze(self.critic(states, new_actions), 1)
